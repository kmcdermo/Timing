...
c_grav(lambda) = alpha(lambda) * lambda * sqrt(ctau), where alpha(lambda) is a scaling constant set for each lambda.
...
I then wrote a script to extract the c_grav, lambda, and width of the neutralino (i.e. the "true" lifetime of the particle, as the old SLHA filenames do not reflect their real lifetime): extract_params.sh. The script outputs a file : width_cgrav.txt.

This file is read-in by: fitcgrav.C, which performs a linear regression for each lambda. The up version uses lambda explicitly in the definition above, the down does not multiply by lambda.
