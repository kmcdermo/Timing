...
void FastSkimmer::MakeListFromTrees()
{
  ...
  for (const auto & SamplePair : Common::SampleMap)
  {
    ...
    // Get File
    const TString filename = Form("%s/%s/%s/%s/%s/%s",Common::eosPreFix.Data(),Common::eosDir.Data(),Common::baseDir.Data(),fInSkimDir.Data(),input.Data(),Common::tupleFileName.Data());
					  ...
					  TH1F *& OutHist, const TString & sample, const TString & treename)
{
  ...
  for (const auto & SamplePair : Common::SampleMap)
  {
    ...
    // Get File
    const TString infilename = Form("%s/%s/%s/%s/%s/%s",Common::eosPreFix.Data(),Common::eosDir.Data(),Common::baseDir.Data(),fInSkimDir.Data(),input.Data(),Common::tupleFileName.Data());
