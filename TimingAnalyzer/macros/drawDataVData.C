...
void SetupInputs(TFile * file, TH1F *& data, TH1F *& sign, const TString & filename_base, const TString & signal, const Color_t color, const TString & label);
...
void MakeLegend(TLegend *& leg, const TString & label_L, const TString & label_R, const TString & signal_label, const TString & extra_R,
		   ...
		   const TString & signal, const TString & finalname)
{
  ...
  SetupInputs(file_L,data_L,sign_L,leftname,signal,kRed+1,"L");

  TFile * file_R = 0; TH1F * data_R = 0, * sign_R = 0;
  SetupInputs(file_R,data_R,sign_R,rightname,signal,kBlue,"R");
  ...
  delete leg;
  MakeLegend(leg,label_L,label_R,signal_label,Form(" (Scaled x %4.1f)",ratio),data_L,data_R,sign_L,sign_R);
...
void SetupInputs(TFile * file, TH1F *& data, TH1F *& sign, const TString & filename_base, const TString & signal, const Color_t color, const TString & label)
{
  ...
  sign->SetLineColor  (color);
  sign->SetMarkerColor(color);
  sign->SetLineStyle(7);
  sign->SetLineWidth(2);
}
		...
		const TH1F * data_L, const TH1F * data_R, const TH1F * sign_L, const TH1F * sign_R)
{
  ...
  leg->AddEntry(data_L,"Data "+label_L,"epl");
  leg->AddEntry(data_R,"Data "+label_R+extra_R,"epl");
