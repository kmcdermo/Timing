...
void CombinePreparer::PrepareCombine()
{
  ...
  for (const auto & HistNamePair : Common::HistNameMap)
  {
    ...
    // get signal hist
    auto SignHist = (TH2F*)fInFile->Get(signhistname.Data());
    Common::CheckValidHist(SignHist,histname,fInFileName);
     
    // WS name
    const auto sample_ws_name = fWSName+"_"+sample;
...
{
  // bkg1 first
  datacard << Form("%s rateParam %s1 bkg %s:%s",fOutFileName.Data(),sample_wsname.Data()) << std::endl;
  ...
  for (const auto & BinRatioVecPair : ABCD::BinRatioVecMap)
  {
    ...
    datacard << Form("%s rateParam %s%d bkg %s:%s",ABCD::bkg1name.Data(),ABCD::binbase.Data(),bin,fOutFileName.Data(),sample_wsname.Data()) << std::endl;

    for (const auto ratio : RatioVec)
    {
      datacard << Form("%s%d rateParam %s%d bkg %s:%s",ABCD::ratiobase.Data(),ratio,ABCD::binbase.Data(),bin,fOutFileName.Data(),sample_wsname.Data()) << std::endl;
    }
  }
}
...
void CombinePreparer::MakeConfigPave()
{
  ...
  fConfigPave->AddText(Form("WS name: %s",fWSName.Data()));
  fConfigPave->AddText(Form("Blind data: %s",Common::PrintBool(fBlindData)));
