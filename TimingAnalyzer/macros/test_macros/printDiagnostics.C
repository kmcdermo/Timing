...
void printDiagnostics(const TString & filename_stat = "", const TString & filename_tot = "", const TString & label = "", const Bool_t maskC = false)
{
  ...
  // do bkgd-only
  dumpBackgrounds("fit_b","Background Only",file_stat,file_tot,output);
  
  // do bkgd+sign
  dumpBackgrounds("fit_s","Background+Signal",file_stat,file_tot,output);
...
void dumpBackgrounds(const TString & fitname, const TString & fitlabel, TFile * file_tot, TFile * file_stat, std::ofstream & output)
{
  ...
  // get vars
  prepVars(fitname,file_stat,varMap_stat,covMap_stat);
  prepVars(fitname,file_tot ,varMap_tot ,covMap_tot);
...
void prepVars(const TString & fitname, TFile * file, std::map<Param,Var> & varMap, std::map<std::pair<Param,Param>,Double_t> & covMap)
{
  const auto & fit = (RooFitResult*)file->Get(fitname.Data());
  const auto & pars_list = fit->floatParsFinal();
  ...
  for (const auto & paramPair_i : paramMap)
  {
    ...
    for (const auto & paramPair_j : paramMap)
    {
      ...
      covMap[{param1,param2}] = fit->correlation(param_name1,param_name2)*varMap[param1].unc*varMap[param2].unc;
      //      std::cout << param_name1.Data() << " " << param_name2.Data() << " : " << fit->correlation(param_name1,param_name2) << " = " << covMap.at({param1,param2}) << std::endl;
    }
  }

  // const auto & covMat = fit->covarianceMatrix();
