...
struct PlotInfo
{
  ...
  PlotInfo(const TString & name, const TString & title, 
	   const TString & corr, const TString & uncorr, const TString & cut,
	   const Int_t nbinsX, const Float_t lowX, const Float_t highX) 
    : name(name), title(title), corr(corr), uncorr(uncorr), cut(cut),
...
void compareMETspectra(const TString & indir, const TString & outfiletext)
{
  ...
  {
    {"inclusive","p_{T}^{miss} [GeV];p_{T}^{miss} [GeV];Fraction of Events","t1pfMETpt","t1pfMETptUncorr","",50,0,2000},
    {"oot","p_{T}^{miss} [GeV];p_{T}^{miss} [GeV];Fraction of Events","t1pfMETpt","t1pfMETptUncorr","(phoisOOT_0==1||phoisOOT_1==1||phoisOOT_2==1||phoisOOT_3==1)",50,0,2000},
  };
  ...
  for (const auto & sample : samples)
  {
    ...
    for (const auto & plotInfo : plotInfos)
    {
      ...
      // read trees
      tree->Draw(Form("%s>>%s",plotInfo.uncorr.Data(),before_hist->GetName()),plotInfo.cut.Data(),"goff");
      tree->Draw(Form("%s>>%s",plotInfo.corr  .Data(),after_hist ->GetName()),plotInfo.cut.Data(),"goff");
      ...
      // legend
      auto leg = new TLegend(0.40,0.69,0.63,0.86);
