...
{
  TFile * file = new TFile(Form("output/recoskim/novid-sph-smin/njets%i/jetpt%i/plots.root",injets,ijetpt),"UPDATE");
  file->cd();
...
//   std::vector<Color_t> colors  = {kBlack,kRed+1,kViolet-1,kBlue+1};
  ...
  for (UInt_t isample = 0; isample < samples.size(); isample++)
  {
    ...
    {
      TH1F * hist = new TH1F(Form("eff_%s_phpt%i",samples[isample].Data(),phpts[jphpt]),Form("Efficiency vs. VID Selection [Jet p_{T} > %i GeV/c, nJets #geq %i]",ijetpt,injets),6,0,6);
