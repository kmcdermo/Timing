void effStackPlot_VID(Int_t ijetpt, Int_t injets)
{
  ...
  TFile * file = TFile::Open(Form("output/recoskim/njets%i/jetpt%i/plots.root",injets,ijetpt));
  file->cd();
  ...
  canv->SaveAs(Form("eff_vid_jetpt%i_njets%i.png",ijetpt,injets));

  TCanvas * canvL = new TCanvas(); canvL->cd();
  leg->Draw();
  if (ijetpt == 0 && injets == 0) canvL->SaveAs("eff_leg_VID.png");
