...
void RECOSkim_PAT::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) 
{
  ...
  if (photonsH.isValid()) // standard handle check
  {
    ...
    for (std::vector<pat::Photon>::const_iterator phiter = photons.begin(); phiter != photons.end(); ++phiter) // loop over photon vector
    {
      ...
      if ( recHits->size() != 0 ) // unfortunately miniAOD is annoying and only saves some of the recHits
      { 
	...
	// radius of semi-major,minor axis is actually the inverse square root of the eigenvalues of the covariance matrix
