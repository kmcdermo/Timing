...
void computePUWeights(const TString & text)
{
  ...
  Common::CheckValidTH1F(dataHist,dataname,filename);

  const TString bkgdname = "Bkgd_Hist";
  TH1F * bkgdHist = (TH1F*)file->Get(Form("%s",bkgdname.Data()));
  Common::CheckValidTH1F(bkgdHist,bkgdname,filename);
  ...
  // clone and divide
  TH1F * PUWeights = (TH1F*)dataHist->Clone(Form("%s",Common::puwgtHistName.Data()));
