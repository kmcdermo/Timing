...
fragdir=${2:-"plot_config/fragments"}
timefitdir=${3:-"timefit_config"}
...
do echo ${!pho} | while read -r index pho_label
	...
	do 
		...
		for era in "${eras[@]}"
		do
		    ./scripts/runEraTreePlotter.sh "${skimdir}/${infile}.root" "${skimdir}/${insigfile}.root" "${cut}" "${varwgtconfigdir}/${varwgtmap}.${inTextExt}" "${plot}" "${miscconfigdir}/${misc}.${inTextExt}" "${outfile}" "${era}" "${outdir}"
		    
		    ./scripts/runEraTreePlotter2D.sh "${skimdir}/${infile}.root" "${skimdir}/${insigfile}.root" "${cut}" "${varwgtconfigdir}/${varwgtmap}.${inTextExt}" "${plot2D}" "${miscconfigdir}/${misc}.${inTextExt}" "${outfile2D}" "${era}" "${outdir}"

		    ./scripts/runTimeFitter.sh "${outfile2D}_${era}.root" "${plot2D}" "${timefitdir}/${timefitconfig}.${inTextExt}" "${outdir}"
