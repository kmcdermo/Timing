...
void wgttime()
{
  ...
  // output histograms
  TH1F * h_seedtime    = new TH1F("h_seedtime",Form("%s seed time",el.Data()),200,-25.0,25.0); h_seedtime->Sumw2();
  TH1F * h_seedtimeTOF = new TH1F("h_seedtimeTOF",Form("%s seed time [TOF]",el.Data()),100,-5.0,5.0); h_seedtimeTOF->Sumw2();
  TH1F * h_rhtimes     = new TH1F("h_rhtimes",Form("%s recHits (E > 1 GeV) time",el.Data()),200,-25.0,25.0); h_rhtimes->Sumw2();
  TH1F * h_rhtimesTOF  = new TH1F("h_rhtimesTOF",Form("%s recHits (E > 1 GeV) time [TOF]",el.Data()),200,-25.0,25.0); h_rhtimesTOF->Sumw2();
  TH1F * h_allE        = new TH1F("h_allE",Form("%s all ",el.Data()),100,0.0,100.0); h_allE->Sumw2();
  TH1F * h_diffE       = new TH1F("h_diffE",Form("%s maxE - seedE",el.Data()),100,0.0,20.0); h_diffE->Sumw2();
  ...
  TH1F * h_nrh_sc30     = new TH1F("h_nrh_sc30",Form("%s nRecHits (SCE > 30 GeV)",el.Data()),200,0,200); h_nrh_sc30->Sumw2();
  TH1F * h_nrh_sc30_rh1 = new TH1F("h_nrh_sc30_rh1",Form("%s nRecHits (SCE > 30 GeV, rhE > 1 GeV)",el.Data()),200,0,200); h_nrh_sc30_rh1->Sumw2();
  ...
  for (UInt_t entry = 0; entry < tree->GetEntries(); entry++)
  {
    ...
    if (std::abs(seedeta) > etaEB || nrh < 0 || scE < 30) continue;
